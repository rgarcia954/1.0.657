...
void Master_CallBack(uint32_t event)
{
    ...
    switch (event)
    {
        ...
        case ARM_SPI_EVENT_TRANSFER_COMPLETE:
        {
            ...
            Driver_SPI0.Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_INACTIVE);
        }
        break;
    ...
    Sys_Delay(5 * SystemCoreClock); /* 5 seconds delay */

    /* Activate SSEL line and start transfer on SPI0/master */
    Driver_SPI0.Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_ACTIVE);
...
int main(void)
{
    ...
    /* Power-up the master SPI interface and register the callback function */
    Driver_SPI0.Initialize(Master_CallBack);
