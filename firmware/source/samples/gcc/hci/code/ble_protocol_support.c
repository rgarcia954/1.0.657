...
/* ----------------------------------------------------------------------------
 * Function      : Device_BLE_Param_Get(uint8_t param_id, uint8_t *lengthPtr, uint8_t *buf)
 * ----------------------------------------------------------------------------
 ...
 *                 - lengthPtr      - Pointer to the length of parameter
 *                 - buf            - Pointer to the returned value
 * Outputs       : - Return value   - Indicate if requested parameter
 *                                    is provided by application
 * ------------------------------------------------------------------------- */
uint8_t Device_BLE_Param_Get(uint8_t param_id, uint8_t *lengthPtr, uint8_t *buf)
{
    uint8_t status = PARAM_OK, i, *ptr;
    uint32_t temp;
    ...
    switch (param_id)
    {
        case PARAM_ID_BD_ADDRESS:
        {
            ...
            for (i = 0; i < BD_ADDR_LEN; i++)
            {
                if (ptr[i] != 0xff)
                {
                    memcpy(buf, ptr, *lengthPtr);
                    break;
                }
            }
            if (i == BD_ADDR_LEN)
            {
                uint8_t tempBDAddrs[BD_ADDR_LEN] = APP_PUBLIC_ADDRESS;
                memcpy(buf, tempBDAddrs, BD_ADDR_LEN);
            }
        }
        break;
        ...
        case PARAM_ID_LPCLK_NO_XTAL32K:
        {
            ...
            buf[0] = false;
            #else  /* if (LPCLK_STANDBYCLK_SRC == LPCLK_SRC_XTAL32) */
            buf[0] = true;
            LPCLK_PeriodValue_Set(LPCLK_PERIOD_VALUE);
            #endif /* if (LPCLK_STANDBYCLK_SRC == LPCLK_SRC_XTAL32) */
        }
        break;
