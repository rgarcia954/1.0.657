...
//SWMMN-270
/// Channel map assessment structure
struct channel_map_assess_tag
{
	/// Channel map update period (in sec)
    uint16_t channel_map_update_period;
	/// RSSI interference threshold (dBm)
    int8_t rssi_interf_thr;
	/// Assessment valid timer (in slot)
    uint32_t assess_valid_timer;
	/// Reassessment timer (in slot)
    uint32_t Reassess_timer;
	/// Maximum counter value for channel assessment
    int8_t assess_count_max;
	/// Minimum counter value for channel assessment
    int8_t assess_count_min;
	/// Counter threshold to consider a channel bad
    int8_t assess_count_thr_bad;
};
...
//SWMMN-270
extern struct channel_map_assess_tag channel_map_assess;
uint8_t BLE_Link_Metrics(uint8_t en,  uint8_t con_hdl, struct ble_link_metrics *metricsPtr);
