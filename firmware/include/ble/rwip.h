...
//SWMMN-1817
struct ble_con_evt_param_t
{
   uint32_t next_anchor_hslot;
   uint32_t next_anchor_hus;
   uint32_t accessAdrs;
   uint16_t current_con_evt_counter;
   uint16_t evt_cnt;
   uint16_t con_intrvl;
   uint8_t crc_init[3];
   uint8_t m_sca;
   uint8_t hop_inc;
   uint8_t last_unmapped_chnl;
   uint8_t chnl_map[5];
   uint8_t num_usedChnl;
   uint8_t phy_rx;
   uint8_t phy_tx;
};
...
{
	/// From application to the stack parameters
	/// 0: means no sleep, 1: means request to go to sleep
	uint8_t app_sleep_request;
	/// Maximum allowed time that stack can go to sleep in half slot, zero means infinite sleep duration
	uint32_t max_sleep_duration;
	/// Minimum allowed time that stack can go to sleep in us.
	uint32_t min_sleep_duration;
	/// From the stack to application parameters
	/// Time in low power clock that the stack has calculated and written in BB->DEEPSLWKUP register
	uint32_t calculated_sleep_duration;
...
/**
 ****************************************************************************************
 * @brief Set ble_baseband_is_awake_flag to indicate BLE is awake
 ****************************************************************************************
 */
void BLE_Baseband_Is_Awake_Flag_Set(void);

/**
 ****************************************************************************************
 * @brief Clear ble_baseband_is_awake_flag to indicate BLE is not awake
 ****************************************************************************************
 */
void BLE_Baseband_Is_Awake_Flag_Clear(void);

/**
 ****************************************************************************************
 * @brief Read ble_baseband_is_awake_flag to check if BLE is awake
 ****************************************************************************************
 */
bool BLE_Baseband_Is_Awake(void);

/* ----------------------------------------------------------------------------
 * Function      : void LPCLK_PeriodValue_Set(float value)
 * ----------------------------------------------------------------------------
 * Description   : Set the period value of low power clock
 * Inputs        : - value    - RC clock or external clock period value in us
 * Outputs       : None
 * Assumptions   : None
 * ------------------------------------------------------------------------- */
void LPCLK_PeriodValue_Set(float value);

//SWMMN-1704
extern void (*BLE_RxStatus_CallBack)(uint8_t *conidx, uint16_t *status, uint8_t *rssi, uint8_t *chnl, uint16_t *length);
void BLE_Set_RxStatusCallBack(void *callBack);
extern void (*BLE_ScanConIndStatus_CallBack)(uint8_t *type, uint8_t *rssi, uint8_t *chnl);
void BLE_Set_ScanConIndStatusCallBack(void *callBack);
//SWMMN-2251
void BLE_Set_RFOffSeqMode(uint8_t mode);

//SWMMN-1817
uint8_t BLE_Extract_ConParam(uint8_t actidx, struct ble_con_evt_param_t *param);

//SWMMN-1817
extern void (*BLE_ChnlMap_Ind_CallBack)(uint8_t *actidx);
void BLE_Set_ChnlMapIndCallBack(void *callBack);
extern void (*BLE_Con_Ind_CallBack)(uint8_t *actidx);
void BLE_Set_ConEstablishCallBack(void *callBack);
