...
static inline void Sys_Clocks_RCSystemClkConfig(uint32_t cfg, uint32_t rc_cfg)
{
    ...
    FLASH0->DELAY_CTRL = (FLASH0->DELAY_CTRL & ~FLASH_DELAY_CTRL_SYSCLK_FREQ_Mask) |
                         FLASH_DELAY_FOR_SYSCLK_48MHZ;
    FLASH1->DELAY_CTRL = (FLASH1->DELAY_CTRL & ~FLASH_DELAY_CTRL_SYSCLK_FREQ_Mask) |
                         FLASH_DELAY_FOR_SYSCLK_48MHZ;
    FLASH0->RESERVED0[0] = ((FLASH0->RESERVED0[0] & ~0x7) | 2);
    FLASH1->RESERVED0[0] = ((FLASH1->RESERVED0[0] & ~0x7) | 2);
 ...
 *                                 as clock and it's prescale value; use
 ...
 *
 *              Note: If SENSOR_CLK is configured to be derived from the RTC
 *                    timer clock the SENSOR_CLK divisor set by this function
 *                    is not used.
 ...
 * @examplecode HAL_examples.c Sys_Clocks_DividerConfig_Example
 */
